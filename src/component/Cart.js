  ...
  useEffect(() => {
    if (isLoggedIn) {
      const fetchCartItems = async () => {
        setloading(true);
        try {
          const res = await fetch("https://backend-production-9640.up.railway.app/api/cart/cartuser", {
            method: "GET",
            headers: {
  ...
  const fetchProductData = useCallback(async () => {
    ...
    try {
      const productDataPromises = cartItems.map((item) =>
        fetch(
          `https://backend-production-9640.up.railway.app/api/product/fetchproduct?_id=${item.productId}`
        ).then((res) => res.json())
      );
  ...
  const handleRemoveCart = async (itemId, e) => {
    ...
    try {
      const response = await fetch(
        `https://backend-production-9640.up.railway.app/api/cart/removefromcart/${itemId}`,
        {
          method: "DELETE",
          headers: {
  ...
  const handleQuantityChange = async (itemId, quantity, price) => {
    ...
    try {
      const res = await fetch(
        `https://backend-production-9640.up.railway.app/api/cart/updatecart/${itemId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
